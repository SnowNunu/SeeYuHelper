...
+ (NSString *)createdAtTimeWithSourceDate:(NSDate *)sourceDate{
    if (SYObjectIsNil(sourceDate)) return @"";
    // 判断是否为今年
    if (sourceDate.sy_isThisYear) {
        if (sourceDate.sy_isToday) { // 今天
            NSDateComponents *cmps = [sourceDate sy_deltaWithNow];
        ...
        } else if (sourceDate.sy_isYesterday) { // 昨天
